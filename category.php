...
if (!$smarty->is_cached($dwt_name.'.dwt', $cache_id))
{
	...
	{
		$goodslist[$key]['goods_name'] = ellipsis($goodslist[$key]['goods_name'],6);
	}
	
	$category_page_banner = get_category_banner_xml('/banner/category_page_banner/'.$cat_id.'/category_page_banner_'.$cat_id.'.xml');
	
    $smarty->assign('category_page_banner',       $category_page_banner);	
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
	...
	$where = "g.is_on_sale = 1 AND g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';
...
function get_cagtegory_goods_count($children, $brand = 0, $min = 0, $max = 0, $ext='')
{
    ...
    /* 返回商品总数 */
	$count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('goods') . " AS g WHERE $where $ext");
    return $count;
}
