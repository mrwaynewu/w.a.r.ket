...
if($_REQUEST['act'] == 'wxconfig'){
    ...
    $filter['start'] = $start;
    $filter['type'] = 'qr';
    $ret = $db -> getAll("SELECT * FROM `wxch_qr` WHERE `type` = 'qr' LIMIT $filter[start] , $filter[page_size]");
    $wxchdata = array();
    foreach($ret as $k => $v){
    ...
    $filter['start'] = $start;
    $filter['type'] = 'qr';
    $ret = $db -> getAll("SELECT * FROM `wxch_qr`  WHERE `affiliate` !=0 LIMIT $start , $filter[page_size]");
    $wxchdata = array();
    foreach($ret as $k => $v){
        if($v['action_name'] == 'QR_LIMIT_SCENE'){
            $v['type_name'] = '永久';
