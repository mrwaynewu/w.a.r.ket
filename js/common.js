...
function addToCartShowDivResponse(result)
{
  ...
  else
  {
	...
	else
	{
		...
		console.dir(document.minitop);
		$("#addtocartdialog").css("top",document.minitop-280);
		$("#addtocartdialog").show();	
	}
  }
  var num = $("#hidden_cart_num").html();
		if (num > 0){
...
{
  var comment_type_good_medium_bad = '';
  var active_tab = 'all_comments';
  if (document.getElementById('comment_tab_normal').value == 1){comment_type_good_medium_bad = 'comment_tab_normal';active_tab = 'all_comments';}
  else if (document.getElementById('comment_tab_good').value == 1){comment_type_good_medium_bad = 'comment_tab_good';active_tab = 'good_rank';}
  else if (document.getElementById('comment_tab_medium').value == 1){comment_type_good_medium_bad = 'comment_tab_medium';active_tab = 'medium_rank';}
  else if (document.getElementById('comment_tab_bad').value == 1){comment_type_good_medium_bad = 'comment_tab_bad';active_tab = 'bad_rank';}
  Ajax.call('comment.php?act=gotopage', 'page=' + page + '&id=' + id + '&type=' + type + '&comment_type_good_medium_bad=' + comment_type_good_medium_bad + '&active_tab=' + active_tab, gotoPageResponse, 'GET', 'JSON');
}
...
function gotoBuyPageResponse(result)
{
	...
	$('#buy-record-pagination').twbsPagination({
		...
		onPageClick: function(event, page) {
			//$('#page-content').text('Page ' + page);
			javascript: gotoBuyPage(page, goods_id);
		}
	});
...
function buyNow(goodsId, parentId, xpbuy)
{
  var goods        = new Object();
  var spec_arr     = new Array();
  var fittings_arr = new Array();
  ...
  Ajax.call('flow.php?step=add_to_cart', 'goods=' + $.toJSON(goods), buyNowResponse, 'POST', 'JSON');
