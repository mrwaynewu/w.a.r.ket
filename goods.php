...
//showr($pictures_thumbnail_section);		
		...
		// custom spec //
		if (empty($properties['pro'])){$properties['pro'] = array();}
		else
		{
			if (empty($properties['pro']['商品属性'])){$properties['pro']['商品属性'] = array();}
		}

		$custom_spec_array = array();
		$custom_spec_array['name_ch'] = '';
		$custom_spec_array['name_en'] = '';		
		$custom_spec_array['brand'] = '';
		$custom_spec_array['weight'] = '';
		$custom_spec_array['preserve_time'] = '';
		$custom_spec_array['preserve_method'] = '';
		$custom_spec_array['manufactory'] = '';


		foreach ($custom_spec_array as $key => $value)
		{
			
			if ($key == 'name_ch')
			{
				$custom_spec_array[$key]['name'] = '名称(中文)';
				$custom_spec_array[$key]['value'] = $goods['goods_name'];
				$properties['pro']['商品属性'][] = $custom_spec_array[$key];				
			}
			else if ($key == 'name_en')
			{
				if ($goods['goods_name_en'] != '')
				{
					$custom_spec_array[$key]['name'] = '名称(英文)';
					$custom_spec_array[$key]['value'] = $goods['goods_name_en'];
					$properties['pro']['商品属性'][] = $custom_spec_array[$key];					
				}
			}
			else if ($key == 'manufactory')
			{
				if ($goods['goods_manufactory'] != '')
				{
					$custom_spec_array[$key]['name'] = '制造商';
					$custom_spec_array[$key]['value'] = $goods['goods_manufactory'];
					$properties['pro']['商品属性'][] = $custom_spec_array[$key];					
				}
			}			
			else if ($key == 'brand')
			{
				//$brand_name = get_brand_name($goods['brand_id']);
				$custom_spec_array[$key]['name'] = '品牌';
				$custom_spec_array[$key]['value'] = $goods['goods_brand'];
				$properties['pro']['商品属性'][] = $custom_spec_array[$key];				
			}
			else if ($key == 'weight')
			{
				if ($brand_name != ''){
					$custom_spec_array[$key]['name'] = '重量';
					$custom_spec_array[$key]['value'] = $goods['goods_weight'];
					$properties['pro']['商品属性'][] = $custom_spec_array[$key];					
				}
			}
			else if ($key == 'preserve_time')
			{
				if ($goods['preserve_time'] != '')
				{
					$custom_spec_array[$key]['name'] = '保质期';
					$custom_spec_array[$key]['value'] = $goods['preserve_time'];
					$properties['pro']['商品属性'][] = $custom_spec_array[$key];					
				}
			}
			else if ($key == 'preserve_method')
			{
				if ($goods['preserve_method'] != '')
				{
					$custom_spec_array[$key]['name'] = '存储方法';
					$custom_spec_array[$key]['value'] = $goods['preserve_method'];
					$properties['pro']['商品属性'][] = $custom_spec_array[$key];
				}
			} 			
		}
		...
		//$properties['pro']['商品属性'][] = $custom_spec_array; 
		//showr($properties['pro']['商品属性']);
		...
		$smarty->assign('tab_active_class_shang_pin_xiang_qing',     'active'); 
...
{
 $sql= "select goods_func_content".$_SESSION['language']." as goods_func_content, goods_func_order from ".$GLOBALS['ecs']->table('goods_func')."where goods_id ='".$goods_id."' order by goods_func_order LIMIT 4";
