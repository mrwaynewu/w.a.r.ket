...
{	
	...
	if($type=='qq'){
		...
		$data=http_build_query($data);
		$get_code_url='https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&'.$data;
		...
		include_once(ROOT_PATH . 'includes/login/facebook/src/facebook.php' );
		...
		$arr['user_id'] = $user_profile['id'];
		$arr['user_name'] = $user_profile['name'];
		$arr['link'] = $user_profile['link'];
		$arr['sex']  = $user_profile['gender'] == 'male' ? '1' : '0';
		$arr['rank_id'] = RANK_ID;
		$info = $arr;	
