...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
		...
		$sql   = 'SELECT goods_id, goods_name, goods_thumb, shop_price FROM ' . $GLOBALS['ecs']->table('goods') .
                " WHERE $where AND is_on_sale = 1 AND is_delete = 0 LIMIT 5";
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<li class="goodsimg"><a href="'.$goods['url'].'" target="_blank" class="history_pic"><img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'" /></a><a href="'.$goods['url'].'" target="_blank"  title="'.$goods['goods_name'].'" class="history_name"  style="text-decoration:none;">'.$goods['short_name'].'</a>'.'<p class="history_price" style="text-decoration:none;	color: #F31C1C;">'.'<a  href="'.$goods['url'].'"  style="text-decoration:none;	color: #F31C1C;">'.$goods['shop_price'].'</a>'.'</p></li>';
        }
        $str .= '';
    }
    return $str;
...
function insert_cart_info()
{
    ...
    $output = $GLOBALS['smarty']->fetch('library/right_cart_info.lbi');
    return $output;
}
...
function insert_ads($arr)
{
    ...
    $time = gmtime();
	if (isset($arr['num']) === false || empty($arr['num'])){$num = '';}
...
//showr($bought_notes);
    ...
    {
		$bought_notes[$key]['user_name'] = anonymous_name($bought_notes[$key]['user_name']);
...
//showr($bought_notes);
	...
	$buy_record_pagination = array();
	for ($i = 1; $i <= $pager['page_count']; $i++){$buy_record_pagination[$i] = $i;}
	//showr($pager);
	$buy_record_pagination_js = "<script> 
	 $('#buy-record-pagination').twbsPagination({
        totalPages: ".$page_count.",
        visiblePages: 7,
        onPageClick: function (event, page) {
            //$('#page-content').text('Page ' + page);
			javascript:gotoBuyPage(page,".$arr['id'].");
        }
		//href: '?page={{number}}'
    });
	</script>";
    ...
    $GLOBALS['smarty']->assign('buy_record_pagination', $buy_record_pagination);
    $GLOBALS['smarty']->assign('goods_id', $arrp[id]);
    $GLOBALS['smarty']->assign('buy_record_pagination_js', $buy_record_pagination_js);
...
function insert_comments_list($arr)
{
    ...
    $cmt = assign_comment($arr['id'],          $arr['type']);
	$cmt['comments'] = format_comment($cmt['comments'],$user_id);
	/* */
	$pager      = $cmt['pager']['pager'];
	$page       =  $cmt['pager']['page'];
	$page_count =  $cmt['pager']['page_count'];
	...
	$comment_record_pagination_js = "<script> 
	 $('#comment-record-pagination').twbsPagination({
        ...
        onPageClick: function (event, page) {
            //$('#page-content').text('Page ' + page);
			javascript:gotoPage(page,".$arr['id'].");
        }
		//href: '?page={{number}}'
    });
	</script>"; 
    ...
    $GLOBALS['smarty']->assign('comments',     $cmt['comments']);
    $GLOBALS['smarty']->assign('pager',        $cmt['pager']);
	$GLOBALS['smarty']->assign('comment_record_pagination', $comment_record_pagination);
	$GLOBALS['smarty']->assign('comment_record_pagination_js', $comment_record_pagination_js);
	$GLOBALS['smarty']->assign('tab_active_class_normal_ajax_return', 0);
	$GLOBALS['smarty']->assign('tab_active_class_good_ajax_return', 0);
	$GLOBALS['smarty']->assign('tab_active_class_show_img_ajax_return', 0);	
