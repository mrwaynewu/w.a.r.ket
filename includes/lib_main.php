 ...
 * @return  array
 */
function assign_comment($id, $type, $page = 1, $comment_type_rank = "", $show_img = false)
{			//$show_img = true;
	...
	$rank_condition_count_good = " AND comment_rank >= 4";
	$rank_condition_count_medium = " AND comment_rank >=1 AND comment_rank <= 3";
	$rank_condition_count_bad = " AND comment_rank < 1";
	...
	$good_rank_count =  $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
						" WHERE id_value = '$id' AND comment_type = '$type' AND parent_id = 0". $rank_condition_count_good);
						
	$medium_rank_count =  $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
						" WHERE id_value = '$id' AND comment_type = '$type' AND parent_id = 0". $rank_condition_count_medium);
						
	$bad_rank_count =  $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
						" WHERE id_value = '$id' AND comment_type = '$type' AND parent_id = 0". $rank_condition_count_bad);
	...
	if ($comment_type_rank == "good"){
		$rank_condition = " AND comment_rank >= ". 4;
		$show_img_condition = "";
		$count = $good_rank_count;
	}
	else if ($comment_type_rank == "medium"){
		$rank_condition = " AND comment_rank >= ". 1 ." AND comment_rank <= ". 3;
		$show_img_condition = "";
		$count = $medium_rank_count;
	}
	else if ($comment_type_rank == "bad"){
		$rank_condition = " AND comment_rank < ". 1;
		$show_img_condition = "";
		$count = $bad_rank_count;
	}
	if ($show_img === true){
	...
	if ($comment_type_rank == "" && $show_img === false){
    ...
    $pager['medium_rank_record_count'] = $medium_rank_count;
    $pager['bad_rank_record_count'] = $bad_rank_count;
...
function is_comment_created_by_user($comment_id,$user_id){
...
function is_liked_by_this_user($comment_id,$user_id){		
		 /* 查看点赞记录 */
		$valid = false;
		$sql = "SELECT user_id
		FROM " . $GLOBALS['ecs']->table('comment_like'). 
		...
		foreach ($res as $key => $value){
			...
			if ($value['user_id'] == $user_id){$valid = true;}// already exist
		}	
		return $valid;
}

function format_comment($comment_arry,$user_id){			
			foreach ($comment_arry as $key => $value){
				$sql ="SELECT count(*) FROM " . $GLOBALS['ecs']->table('comment_like'). 
					" WHERE comment_id = ". $value['id'];
					$count = $GLOBALS['db']->getOne($sql);
					$comment_arry[$key]['like'] = $count;
					if (is_liked_by_this_user($value['id'],$user_id) === true){$comment_arry[$key]['is_user_liked'] = 1;}
					else {$comment_arry[$key]['is_user_liked'] = 0;}
			
					$sql ="SELECT comment_img_url, comment_img_thumb_url FROM " . $GLOBALS['ecs']->table('comment_img'). 
					" WHERE comment_id = ". $value['id'];
					$img_arr = $GLOBALS['db']->getAll($sql);
					$comment_arry[$key]['img'] = $img_arr;	
			}
			...
			$comment_arry = $temp;$temp = array();
			return 	$comment_arry;
}

function get_left_goods_nav($id,$type,$is_show=""){//$type: 0 means its based on goods_id, 1 means its based on cat_id
	...
	$cat_arry = get_all_cat_of_goods_info($is_show);// true: is_show == 1;
	//showr($cat_arry);
	$top_cat_arry = array();
...
function get_all_cat_of_goods_info($is_show='')
{	
	if ($is_show == ''){$is_show = false;}
	else {$is_show = true;}
